...
export function AssetReferenceToken({ asset, onRemove }: { asset: AssetRow; onRemove: () => void }) {
  return (
    <span className="asset-token" title={`引用 ID: ${asset.id}`}>
      @{asset.name}
      <Button variant="ghost" onClick={onRemove} aria-label={`移除 ${asset.name} 引用`}>
        ×
      </Button>
    </span>
  );
    ...
    <div className="workspace-grid">
      <Card className="materials">
        <div className="section-title">
          <span>我的素材</span>
          <Button onClick={onCreate}>＋ 创建素材</Button>
        </div>
        <ScrollArea>
          {Object.entries(Object.groupBy(assets, (a) => a.category || '未分类')).map(([category, group]) => {
            const items = group ?? [];
            return (
          ...
          {!assets.length && <div className="empty">还没有素材，先创建一个可复用材料喵</div>}
        </ScrollArea>
      </Card>
      <Card className="task">
        <div className="task-heading">
          ...
          <span className="muted">素材只是引用，不会替换任务文字</span>
        </div>
        <Textarea
          ref={ref}
          value={draft.prompt}
          onChange={(e) =>
            draft.set({
              ...
              referencedAssetIds: draft.referencedAssetIds.filter((id) => e.target.value.includes(`](asset:${id})`)),
            })
          }
        ...
        <p className="hint">引用 token 带有稳定素材 ID；可直接编辑文本或点击 × 移除</p>
      </Card>
    </div>
  );
}
