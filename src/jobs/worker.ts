...
export type SingleImageGenerator = (input: { sticker: StickerConfig; model?: string; aspectRatio?: string; imageSize?: string; removeBackground?: boolean }) => Promise<SingleImageResult>;
export interface WorkerOptions { repositories: Repositories; generator?: SingleImageGenerator; outputDir?: string; heartbeatMs?: number; staleAfterMs?: number; maxAttempts?: number; }
...
export class GenerationWorker {
  private readonly repo: Repositories; private readonly generate: SingleImageGenerator; private readonly outputDir: string;
  private readonly heartbeatMs: number; private readonly staleAfterMs: number; private readonly maxAttempts: number;
  private stopped = false; private active?: Promise<void>; private loop?: Promise<void>;
  constructor(options: WorkerOptions) { this.repo = options.repositories; this.generate = options.generator ?? generateSingleImage; this.outputDir = options.outputDir ?? join(import.meta.dir, "../../output"); this.heartbeatMs = options.heartbeatMs ?? 15_000; this.staleAfterMs = options.staleAfterMs ?? this.heartbeatMs * 2; this.maxAttempts = options.maxAttempts ?? 3; }
  ...
  private async process(itemId: string): Promise<void> {
    ...
    try {
      ...
      const result = await this.generate({ sticker: { name: job.assetName, prompt: job.promptSnapshot, referenceImages: [] }, model: typeof options.model === "string" ? options.model : undefined, aspectRatio: typeof options.aspectRatio === "string" ? options.aspectRatio : undefined, imageSize: typeof options.imageSize === "string" ? options.imageSize : undefined, removeBackground: typeof options.removeBackground === "boolean" ? options.removeBackground : undefined });
      if (!result.success || !result.imageBuffer) throw new Error(result.error ?? "generator returned no image");
      await mkdir(this.outputDir, { recursive: true }); const mimeType = result.mimeType ?? "image/png"; const actualName = `${item.jobId}-${item.ordinal}${extension(mimeType)}`; finalPath = join(this.outputDir, actualName); tempPath = `${finalPath}.${item.id}.tmp`;
      await Bun.write(tempPath, result.imageBuffer); await rename(tempPath, finalPath); tempPath = undefined;
      this.repo.finalizeRequest({ requestId: request.id, itemId, status: "succeeded", file: { fileName: actualName, mimeType, sizeBytes: result.imageBuffer.byteLength } });
    } catch (error) {
