...
export function createRepositories(db: Db) {
  ...
  return {
    ...
    cancelJob: (jobId:string) => db.transaction(tx=>{const current=tx.select().from(jobs).where(eq(jobs.id,jobId)).get(); if (!current) return undefined; if (!["queued","running"].includes(current.status)) return current; const t=nowUtc(); tx.update(items).set({status:"cancelled",finishedAt:t,heartbeatAt:null}).where(and(eq(items.jobId,jobId),sql`${items.status} = 'queued' OR (${items.status} = 'running' AND NOT EXISTS (SELECT 1 FROM llm_requests WHERE llm_requests.item_id = ${items.id} AND llm_requests.status = 'running'))`)).run(); tx.update(jobs).set({status:"cancelled",cancelledAt:t,updatedAt:t}).where(eq(jobs.id,jobId)).run(); addEvent(tx,jobId,"job.cancelled"); refreshJob(tx,jobId,t); return tx.select().from(jobs).where(eq(jobs.id,jobId)).get();}),
