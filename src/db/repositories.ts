...
export function createRepositories(db: Db) {
  ...
  return {
    ...
    startRequest: (itemId: string) => db.transaction((tx) => { const item=tx.select().from(items).where(eq(items.id,itemId)).get(); if (!item) transitionError(`item ${itemId} not found`); if (item.status !== "running") transitionError(`start request requires running item, got ${item.status}`); const previous=tx.select({max:sql<number>`max(${requests.attempt})`}).from(requests).where(eq(requests.itemId,itemId)).get(); const attempt=(previous?.max ?? 0)+1, row={id:id(),itemId,attempt,status:"running" as const,startedAt:nowUtc()}; tx.insert(requests).values(row).run(); return row; }),
    finishRequest: (requestId: string, status: Exclude<RequestStatus,"running">, error?: string) => db.transaction((tx) => { const current=tx.select().from(requests).where(eq(requests.id,requestId)).get(); if (!current) transitionError(`request ${requestId} not found`); if (current.status !== "running") transitionError(`finish request requires running request, got ${current.status}`); return tx.update(requests).set({status,error,finishedAt:nowUtc()}).where(and(eq(requests.id,requestId),eq(requests.status,"running"))).returning().get(); }),
    finishItem: (itemId: string, status: Exclude<ItemStatus,"queued"|"running">, error?: string) => db.transaction(tx=>{const current=tx.select().from(items).where(eq(items.id,itemId)).get(); if (!current) transitionError(`item ${itemId} not found`); if (current.status !== "running") transitionError(`finish item requires running item, got ${current.status}`); const row=tx.update(items).set({status,error,finishedAt:nowUtc(),heartbeatAt:null}).where(and(eq(items.id,itemId),eq(items.status,"running"))).returning().get(); if(row){const at=nowUtc(); refreshJob(tx,row.jobId,at); addEvent(tx,row.jobId,`item.${status}`,itemId,error);} return row;}),
