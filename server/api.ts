...
function parseMetadata(value: unknown): Record<string, unknown> {
  if (!value || typeof value !== 'object' || Array.isArray(value)) throw new InputError('metadata must be an object');
...
function publicJob(repo: Repositories, id: string) {
  ...
  if (!result.job) return undefined;
  return {
    ...result.job,
  ...
  return async function handler(req: Request): Promise<Response> {
    ...
    try {
      ...
      if (parts[1] === 'assets') {
        ...
        if (parts.length >= 3 && validId(parts[2])) {
          ...
          if (req.method === 'PATCH' && parts.length === 3) {
            ...
            if (b.name !== undefined) input.name = text(b.name, 'name', 200);
            if (b.prompt !== undefined) input.prompt = text(b.prompt, 'prompt', MAX_PROMPT);
            if (b.category !== undefined) input.category = text(b.category, 'category', 100);
            if (b.metadata !== undefined) input.metadata = parseMetadata(b.metadata);
            if (!Object.keys(input).length) throw new InputError('at least one field is required');
      ...
      if (parts[1] === 'jobs') {
        ...
        if (req.method === 'POST' && parts.length === 2) {
          ...
          const refs = rawRefs.map((v) => {
            ...
            if (!a || a.archivedAt) throw new InputError('asset not found');
            return {
              id: a.id,
              name: a.name,
              prompt: a.prompt,
      ...
      if (req.method === 'GET' && parts[1] === 'output' && parts.length === 3 && parts[2]) {
        ...
        if (!records.length) return error(404, 'NOT_FOUND', 'output not found');
        const target = resolve(outputDir, fileName);
        if (!target.startsWith(outputDir + '/')) return error(404, 'NOT_FOUND', 'output not found');
        try {
          await stat(target);
          return new Response(Bun.file(target), { headers: { 'content-type': records[0]!.mimeType } });
        } catch {
          return error(404, 'NOT_FOUND', 'output not found');
        }
      }
      ...
      if (e instanceof InputError) return error(400, 'INVALID_INPUT', e.message);
      if (e instanceof SyntaxError) return error(400, 'INVALID_INPUT', 'invalid request');
