...
describe("web API", () => {
  ...
  test("startServer serves and processes accepted jobs through its background worker", async () => {
    ...
    const app = await startServer({ databasePath: ":memory:", outputDir, port: 0, generator: async () => ({ success: true, imageBuffer: Buffer.from("fake"), mimeType: "image/png" }) });
    try {
      const base = `http://${app.server.hostname}:${app.server.port}`;
      const accepted = await fetch(`${base}/api/jobs`, { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify({ prompt: "server integration" }) });
  ...
  test("mutation routes reject extra path segments", async () => {
    const db = await openDatabase(":memory:"); const repo = createRepositories(db.db); const api = createApiHandler({ repositories: repo });
    const asset = await (await api(request("POST", "/api/assets", { name: "A", prompt: "hello" }))).json() as { id: string };
    const job = await (await api(request("POST", "/api/jobs", { prompt: "x" }))).json() as { id: string };
    for (const [method, path, value] of [["PATCH", `/api/assets/${asset.id}/extra`, { name: "changed" }], ["DELETE", `/api/assets/${asset.id}/extra`], ["POST", `/api/assets/${asset.id}/archive/extra`], ["POST", `/api/jobs/${job.id}/cancel/extra`], ["POST", `/api/jobs/${job.id}/retry-failed/extra`]] as const) {
