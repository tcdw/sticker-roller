绘制一张二次元风格的 Telegram Sticker。

## 基础注意事项

- 使用的人物是我的自设，人物特征：anime style, long white hair in ponytail, thin dark gray scrunchie, ahoge, bright blue eyes, slight blush, orange hoodie, black turtleneck, slim black pants, black boots
- 风格为 Q 版
- 放在一张白色背景的图片上
- 不要附带任何文字
- 线条需要略粗一点，以应对图片被缩小显示的情况
- 只需要绘制头部就可以，但是不要粗暴的将底部裁切，也不要使用渐变

## 具体需求

我的自设抱住图中的人物。注意两位在图中的身高应该差不多。人物均以半身呈现。

需要显现出我的自设（二次元风格）和图中抽象人物 3D 建模风的风格冲击感。
